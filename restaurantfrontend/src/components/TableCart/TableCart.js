...
return(
    ...
    const showTotalBill=()=>{
        return(
         ...
         <Grid  item xs={6}>
         <Button onClick={()=>handlePayment((totalAmount-totalOffer)+((totalAmount-totalOffer)*admin.gsttype/100))} variant="contained" style={{display:'flex',marginLeft:'auto'}} color="primary">Online Payment</Button>
         </Grid>
   ...
   const heading=()=>{
    return(
        ...
        <div>
            ...
            <Grid item xs={6}>
                <TextField onChange={(event)=>setCustomerName(event.target.value)} label="Customer Name" variant="standard" /></Grid>
                <Grid item xs={6}>
                <TextField onChange={(event)=>setMobileNo(event.target.value)} label="Mobile" variant="standard" /></Grid>
