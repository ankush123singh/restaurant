        ...
        const fetchAllFloor=async()=>{
           ...
           const showTable=()=>{
            return table.map((item,i)=>{
                return(<Paper onClick={()=>handleTableClick(item,i)} elevation={3} style={{borderRadius:5,width:80,height:80, display:'flex',justifyContent:'center',alignItems:'center',padding:5,margin:8,background:i==selectedTable?'#d35400':'#fdcb6e',flexDirection:'column',cursor:'pointer'}}>
...
<div style={{fontFamily:'Kanit',fontWeight:'bold',fontSize:16,color:'#fff',padding:5}}>{item.floor}</div>
</Paper>)
